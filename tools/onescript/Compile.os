...
Процедура ЗагрузитьОбработкуИлиОтчетИзXML(ИмяФайла_filename,ИмяОбработкиИлиОтчета, ПутьРаспаковки)
	...
	ИмяФайлаОбработкиИлиОтчета = ТекущийКаталог() + ИмяОбработкиИлиОтчета;
	...
	Если ФайлОбработкаИлиОтчет.Существует() Тогда
		...
		Если ФайлВерсии.Существует() Тогда
			...
			Если НРег(ВерсияСтрокой) = НРег(ТекущаяВерсияСтрокой) Тогда
				Массив.Удалить(0);
				Если ПроверитьЧтоВсеФайлыИсходниковСоответствуютИхВерсиям( Массив, 
					ПутьРаспаковки + ПолучитьРазделительПути() + ИмяКаталога_filename
					) Тогда
	...
	ЗаписатьВерсиюОбработкиИлиОчета(ФайлОбработкаИлиОтчет, ПутьРаспаковки, ИмяКаталога_filename,ИмяОбработкиИлиОтчета);
...
Процедура СобратьОбработкуИлиОтчетВКаталогеИПодКаталогах(Путь, ПутьРаспаковки)
	...
	Файлы = НайтиФайлы(ПутьРаспаковки,"filename.*",Истина);
	...
	Для Каждого Файл Из Файлы Цикл
		...
		ЗагрузитьОбработкуИлиОтчетИзXML(Файл.ПолноеИмя,ИмяОбработкиИлиОтчета, ПутьРаспаковки);
	КонецЦикла;	
...
Иначе
	...
	Если ПродолжитьРаботу Тогда
		...
		СобратьОбработкуИлиОтчетВКаталогеИПодКаталогах(АргументыКоманднойСтроки[0], АргументыКоманднойСтроки[1]);
	КонецЕсли;
КонецЕсли;
